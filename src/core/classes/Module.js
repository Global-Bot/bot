    ...
    _init(global) {
        ...
        for (const { name, type, optional } of this._moduleProps) {
            if (!Object.hasOwnProperty.call(this, name) && typeof this[name] != type && !optional) {
                throw new Error(ensureError(`Module "${this.constructor.name}": Required prop "${name}" does not exist`, name, false, type, typeof this[name]));
            }
            
            if (typeof this[name] != type && !(optional && typeof this[name] == 'undefined')) {
                throw new Error(ensureError(`Module "${this.constructor.name}": Required prop "${name}" is the wrong type, should be "${type}", received: "${typeof this[name]}"`, name, true, type, typeof this[name]));
            }
        }
